...
namespace graph
{
    ...
    public:
        ...
        std::optional<RouteInfo> BuildRoute(VertexId from, VertexId to) const;  // Дать Инфо по пути от и до
    ...
    private:
        ...
        void InitializeRoutesInternalData(const Graph& graph)   // Инициализировать внутренние данные маршрутов
        {
        ...
        void RelaxRoute(VertexId vertex_from, VertexId vertex_to, const RouteInternalData& route_from, const RouteInternalData& route_to)  // Расслабляющий маршрут
        {
        ...
        void RelaxRoutesInternalDataThroughVertex(size_t vertex_count, VertexId vertex_through)     // Relax направляет внутренние данные через вершину
        {
